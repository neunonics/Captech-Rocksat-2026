...
void onMessageSent(uint16_t id, rbMsgStatus_t status) {
...
rbCallbacks_t myCallbacks = {
    .messageProvisioning = nullptr,
    .moMessageComplete = onMessageSent,
    .mtMessageComplete = nullptr,
    .constellationState = nullptr
};
...
void setup(){
    ...
    DEBUGSERIAL.setRx(PA2);
    DEBUGSERIAL.setTx(PA2);
...
void loop(){
    ...
    if (millis() - lastAction >= 1000) {
        DEBUGSERIAL.println("This is the debug message A1");
        loopItteration++;
